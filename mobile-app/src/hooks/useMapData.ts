/**
 * All map data-fetching logic for the mobile app.
 *
 * Ported verbatim from the web dashboard's `hooks/useMapData.ts` (same Supabase
 * views/tables, same client-side caching), adapted only for the RN types. The
 * hazards it returns feed both the Map (pins) and any screen that wants live
 * report counts.
 */
...
function normalizeChoroplethRows(rows: RawChoroplethRow[]): StateChoroplethStat[] {
  return rows
    ...
    .map((row) => ({
      id: row.boundary_id,
      state_name:
        row.state_name ?? row.country_name ?? row.district_name ?? row.boundary_name ?? 'Unknown Area',
      iso_code: row.boundary_id,
      geojson: row.geojson as Geometry,
      ...
      high_severity_ratio: row.total_reports > 0 ? row.severity_high_count / row.total_reports : 0,
    }));
}
